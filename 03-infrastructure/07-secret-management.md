...
**1. Creation** — Generating secrets with sufficient entropy. Database passwords, API keys, TLS certificates all need cryptographically strong random generation, not human-chosen values. Skip this and you get guessable credentials, shared passwords across environments, or secrets derived from predictable patterns.

**2. Storage** — Persisting secrets in an encrypted, access-controlled store with versioning. The secret manager (Vault, GCP Secret Manager, AWS Secrets Manager) is the single source of truth. Skip this and secrets scatter across `.env` files, Slack messages, wikis, and developer laptops with no audit trail.

**3. Distribution** — Delivering secrets to the workloads that need them at runtime. This includes Kubernetes Secrets, environment injection, volume mounts, or sidecar patterns. Skip this (or do it poorly) and secrets get baked into images, hardcoded in deployment manifests, or passed through insecure channels.

**4. Rotation** — Periodically replacing secrets with new values without downtime. This limits the blast radius of a compromise — a leaked credential that rotates every 24 hours is dangerous for hours, not forever. Skip this and compromised credentials remain valid indefinitely.

**5. Revocation** — Immediately invalidating a specific credential when it's compromised or no longer needed (employee leaves, service decommissioned). Skip this and you accumulate long-lived credentials with no expiration.
...
The controller generates a sealing key pair (RSA 4096) on first startup. The public key encrypts; the private key (stored as a K8s Secret in `kube-system`) decrypts.
